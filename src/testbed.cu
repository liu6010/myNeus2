...
// add by lhw:bool invert_normals = true
void Testbed::compute_and_save_marching_cubes_mesh(const char* filename, Vector3i res3d , BoundingBox aabb, float thresh, bool unwrap_it,bool invert_normals) {
	if (aabb.is_empty()) {
	...
	if ((m_testbed_mode == ETestbedMode::Nerf)){
		// add by lhw
		save_mesh(m_mesh.verts, m_mesh.vert_normals, m_mesh.vert_colors, m_mesh.indices, filename, unwrap_it, m_nerf.training.dataset.scale, m_nerf.training.dataset.offset, m_nerf.training.dataset.from_na || invert_normals);
...
void Testbed::imgui() {
	...
	if ((m_testbed_mode == ETestbedMode::Nerf) || m_testbed_mode == ETestbedMode::Sdf) {
		if (ImGui::CollapsingHeader("Marching Cubes Mesh Output")) {
			...
			// add by lhw
			ImGui::SameLine();
			ImGui::Checkbox("invert_normals", &m_invert_normals);
			// add end
			...
			if (uint32_t tricount = m_mesh.indices.size()/3) {
				...
				if (ImGui::Button("Save it!")) {
					// add by lhw
					save_mesh(m_mesh.verts, m_mesh.vert_normals, m_mesh.vert_colors, m_mesh.indices, obj_filename_buf, m_mesh.unwrap, m_nerf.training.dataset.scale, m_nerf.training.dataset.offset, m_nerf.training.dataset.from_na || m_invert_normals);
