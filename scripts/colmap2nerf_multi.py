# Copyright (c) 2020-2022, NVcameraidIA CORPORATION.  All rights reserved.
#
...
def parse_args():
    ...
    parser.add_argument("--video_in", default="", help="run ffmpeg first to convert a provided video file into "
    ...
    parser.add_argument("--text", default="colmap_text", help="input path to the colmap text files "
                                                              "(set automatically if run_colmap is used)")
...
if __name__ == "__main__":
    ...
    with open(os.path.join(TEXT_FOLDER,"cameras.txt"), "r") as f:
        ...
        for line in f:
            ...
            frame = {"camera_angle_x": angle_x, "camera_angle_y": angle_y, "fl_x": fl_x, "fl_y": fl_y, "k1": k1,
                     "k2": k2, "p1": p1, "p2": p2, "cx": cx, "cy": cy, "w": w, "h": h, }

            dic[int(els[0]) - 1]=frame
    ...
    with open(os.path.join(TEXT_FOLDER,"images.txt"), "r") as f:
        ...
        for line in f:
            ...
            if i % 2 == 1:
                ...
                depth_rel = os.path.relpath(DEPTH_FOLDER)
                print(image_rel)
                print(depth_rel)
                cameraid=int(elems[8])-1
                name = str(f"./{image_rel}/{'_'.join(elems[9:])}")
                depth_name = str(f"{depth_rel}/{'_'.join(elems[9:])}")
                depth_name = depth_name[:-3]+"png"
                b=sharpness(name)
                ...
                frame = {"file_path": name, "depth_path": depth_name, "sharpness": b, "transform_matrix": c2w,
                         "camera_angle_x": dic[cameraid]["camera_angle_x"],
                         "camera_angle_y": dic[cameraid]["camera_angle_y"], "fl_x": dic[cameraid]["fl_x"],
                         "fl_y": dic[cameraid]["fl_y"], "k1": dic[cameraid]["k1"], "k2": dic[cameraid]["k2"],
                         "p1": dic[cameraid]["p1"], "p2": dic[cameraid]["p2"], "cx": dic[cameraid]["cx"],
                         "cy": dic[cameraid]["cy"], "w": dic[cameraid]["w"], "h": dic[cameraid]["h"]}
