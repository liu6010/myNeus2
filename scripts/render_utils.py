...
def render_img_training_view(args, testbed, log_ptr, image_dir, frame_time_id = 0, training_step = -1):
    ...
    frame_num = len(test_transforms["frames"])
    print("frame_num:",frame_num)
    for camera_view in range(frame_num):
        print("camera_view:", camera_view)
        frame_time_id = camera_view
        img_path = os.path.join(eval_path, 'images',f'{camera_view:04}')
...
def render_img_training_view2(args, testbed, log_ptr, image_dir, frame_time_id = 0, training_step = -1):
    eval_path = args.output_path
    os.makedirs(eval_path, exist_ok=True)
