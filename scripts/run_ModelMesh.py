...
if __name__ == "__main__":
	...
	file_path = args.scene[:args.scene.rfind("/")]
	args.output_path = os.path.join(root_path,file_path,'output',args.name)
	...
	if args.test:
		...
		if n_steps > 0:
			...
			save_snapshot = os.path.join(args.output_path,"checkpoints",f"{n_steps}.msgpack")
			print("save_snapshot:",save_snapshot)
			save_mesh_path = os.path.join(args.output_path,"mesh",f"{n_steps}.ply")
